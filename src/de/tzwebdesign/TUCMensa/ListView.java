...
public class ListView extends Activity {
	...
	final Runnable completedloading = new Runnable() {
		public void run() {
			...
			build();
			Toast.makeText(ListView.this,R.string.findNewEssen ,Toast.LENGTH_LONG).show();
	...
	void doBindService() {
	    ...
	    bindService(new Intent(ListView.this, MensaService.class), mConnection, Context.BIND_AUTO_CREATE);
	    mIsBound = true;
	    }
	}
	...
	LinearLayout ll;
		...
		while (true) {
			...
			if (sprache.equals("de")) {
				tw0.setText(nf.format(mDay)+"."+nf.format(mMonth)+"."+nf2.format(mYear));
			} else {
				tw0.setText(nf.format(mMonth)+"."+nf.format(mDay)+"."+nf2.format(mYear));
